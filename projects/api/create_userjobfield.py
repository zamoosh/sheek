...
@swagger_auto_schema(method='POST', request_body=UserJobFieldCreateDto,
                     responses={200: UserJobFieldViewDto(many=False)})
@api_view(['POST'])
...
def set_user_jobfied(request):
    ...
    user_jobfield.jobField = request.data.get('jobField', 0)
...
def set_state_expert(request, jobfield):
    ...
    return Response(UserStateSerializer(user_state, many=True), status=HTTP_201_CREATED)
...
def get_state(request, jobfield):
    ...
    return Response(ProvinceSerializer(state, many=True), status=HTTP_200_OK)
